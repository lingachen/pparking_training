...
void init_led(int led_index)
{
	...
	led_sockfd[led_index] = tcp_client_init(led_ipaddress[led_index], 5200, &send_to_led);
	...
	int img_len;
	FILE *fp;
	if (led_index == 0) fp = fopen("/opt/led/ArrowUp.gif", "rb");
	else if (led_index == 1) fp = fopen("/opt/led/ArrowLeft.gif", "rb");
	else					 fp = fopen("/opt/led/ArrowUp.gif", "rb");
...
void send_led(int led_index, char *led_data, int len)
{
	...
	tcp_send(led_sockfd[led_index], led_data, len, &send_to_led);
