...
ui_lot_monitor_page::ui_lot_monitor_page()
{
    ...
    int m = 0;
    if(combo_monitor_mode->currentIndex() == PGS_MODE) m = combo_level->currentIndex();
    total_pages  = max_page_no[combo_monitor_mode->currentIndex()][m] + 1;
    current_page = 0;
...
void ui_lot_monitor_page::combo_selected(int index)
{
    ...
    int m = 0;
    if(combo_monitor_mode->currentIndex() == PGS_MODE) m = combo_level->currentIndex();
    total_pages  = max_page_no[index][m] + 1;
    current_page = 0;
    page_list->setText(QString::number(current_page)+"/"+QString::number(total_pages));
    status->setText(QString::number(w->get_current_page_index()));
...
void ui_lot_monitor_page::update_lot_monitor_images(int combo_index)
{
    ...
    struct data_blk_rec data_blk_rec_local[MAX_BLACKLIST];
    ...
    else if(combo_index == BLK_MODE) total_display_count = minc_pgs_query_data_blk_by_pageindex(pageindex ,&data_blk_rec_local[0]);
    ...
    for(int i=0; i<total_display_count; i++) {
        ...
        else if(combo_index == BLK_MODE) {
            sprintf(display_list[i],"%d",data_blk_rec_local[i].id);
            show_lot_image(i, QString::number(data_blk_rec_local[i].id), QString(data_blk_rec_local[i].platestr), QString(data_blk_rec_local[i].modified_time), QString(data_blk_rec_local[i].car_image));
        }
    }
