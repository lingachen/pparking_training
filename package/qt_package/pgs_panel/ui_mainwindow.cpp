...
void ui_mainwindow::init_data()
{
    ...
    Total_blk_data_count            = minc_pgs_query_blacklist(&out_data_blk_rec[0]);
    ...
    for(int i=0; i<Total_blk_data_count; i++) {
        int val = atoi(out_data_blk_rec[i].pageindex);
        if(max_page_no[BLK_MODE][0] < val) max_page_no[BLK_MODE][0] = val;
    }
    ...
    ret = minc_pgs_init_alarm(Total_data_pgs_count, pgs_watchdog_timer, &out_data_pgs_rec[0]);
...
int ui_mainwindow::get_current_page_index()
{
    ...
    if(n10000 == PAGE_LOT_MONTIOR) n1000 = w->main_tab_page->lot_monitor_page->combo_monitor_mode->currentIndex();
         ...
         if(n10000 == PAGE_LOT_MONTIOR && n1000 == PGS_MODE) n100 = atoi(w->main_tab_page->lot_monitor_page->combo_level->currentText().toStdString().c_str());
...
void ui_mainwindow::udp_receiver_data()
{
    ...
    while(receiver->hasPendingDatagrams()) {
        ...
        // update alarm data table
        if (QString::compare(Mode.toStdString().data(), "PGS", Qt::CaseInsensitive)==0) {
            QString Lotid = json_obj["Lotid"].toString();
            int ret = minc_pgs_update_alarm(true, (char *)Lotid.toStdString().c_str(), "Good");
            if(ret == 0 && current_page_id == PAGE_ALARM*10000) {
                int lotindex = get_pgs_lotid_index((char *)Lotid.toStdString().c_str());
                w->main_tab_page->alarm_tab_page->lot_status[lotindex]->btn_status->setStyleSheet("background-color:green;color:black;");
            }
        }
        ...
        if(current_page_id == PageID.toInt()) {
             if (QString::compare(Mode.toStdString().data(), "PGS", Qt::CaseInsensitive)==0 ||
                 ...
                 // only for BLK
                 if(QString::compare(Mode.toStdString().data(), "ENF", Qt::CaseInsensitive)==0) {
                     int id = PageID.toInt() - 3000;
                     max_page_no[BLK_MODE][0] = id;
                 }
             }

             // for blacklist
             if (QString::compare(Mode.toStdString().data(),"BLK",Qt::CaseInsensitive)==0) {
                 QString Lotid          = json_obj["Lotid"].toString();
