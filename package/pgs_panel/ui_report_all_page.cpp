...
ui_report_all_page::ui_report_all_page(QWidget *parent) : QWidget(parent)
{
    if (this->objectName().isEmpty())
        this->setObjectName(QStringLiteral("ui_report_all_page"));
...
void ui_report_all_page::init_data()
{
    header << "Modified_time" << "Station/lot_id" << "Platenum" << "Ptype" << "Movement_time" << "Car_image" << "Comments";
}

void ui_report_all_page::btn_search_clicked()
{
    ...
    strcpy(station_id, report_input->inp_stationid->text().toStdString().c_str());

    info->clear();
    info->setColumnCount(0);
    info->setRowCount(0);

    struct log_lpr_movement_rec *report_page_log_lpr_movement = (struct log_lpr_movement_rec *)malloc(30000*sizeof(struct log_lpr_movement_rec));
    ...
    int ret_lpr_movement = minc_pgs_query_log_lpr_movement_by_report(inp_datetime_yymmddhh_start, inp_datetime_yymmddhh_stop, inp_platenum, station_id, &report_page_log_lpr_movement[0]);
    int ret_pgs_movement = minc_pgs_query_log_pgs_movement_by_report(inp_datetime_yymmddhh_start, inp_datetime_yymmddhh_stop, inp_platenum, station_id, &report_page_log_pgs_movement[0]);
    info->setColumnCount(7);
    info->setRowCount(ret_lpr_movement+ret_pgs_movement+1);
    info->setHorizontalHeaderLabels( header );
    for(int i=0; i<ret_lpr_movement; i++) {
        info->setItem(i,0,new QTableWidgetItem(QString(report_page_log_lpr_movement[i].modified_time)));   //modified_time
        info->setItem(i,1,new QTableWidgetItem(QString(report_page_log_lpr_movement[i].station_id)));      //station_id
        info->setItem(i,2,new QTableWidgetItem(QString(report_page_log_lpr_movement[i].platenum)));        //platenum
        info->setItem(i,3,new QTableWidgetItem(QString(report_page_log_lpr_movement[i].ptype)));           //ptype
        info->setItem(i,4,new QTableWidgetItem(QString(report_page_log_lpr_movement[i].movement_time)));   //movement_time
        info->setItem(i,5,new QTableWidgetItem(QString(report_page_log_lpr_movement[i].car_image)));       //car_image
        info->setItem(i,6,new QTableWidgetItem(QString(report_page_log_lpr_movement[i].comments)));        //comments
        info->resizeColumnsToContents();
    }
    free(report_page_log_lpr_movement);
    for(int i=0; i<ret_pgs_movement; i++) {
...
void ui_report_all_page::btn_export_clicked()
{

}

void ui_report_all_page::info_record_clicked(int row, int col)
{
