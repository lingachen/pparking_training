...
public:
    ...
    /* Retrieve predict calulated result
     *
     * image: Input image, classid: predict class, float *p => confident level (for all)
     *
     */
    bool predict(const Image &image, int class_num, int *classid, int *confident_val_int);
